...
if (!console._skpmEnabled) {
  var debuggerId = require('@skpm/sketch-debugger/debugger-identifier')
  var actions = require('@skpm/sketch-debugger/shared-actions')

  var threadDictionary = NSThread.mainThread().threadDictionary()

  function isDebuggerPresent() {
    return !!threadDictionary[debuggerId]
  }
  ...
  function sendToDebugger(name, payload) {
    ...
    var webview = threadDictionary[debuggerId].contentView().subviews()
    webview = webview[webview.length - 1]

    return webview.stringByEvaluatingJavaScriptFromString('sketchBridge(' + JSON.stringify({name, payload}) + ');')
  }
  ...
  function logEverywhere (type, args) {
    ...
    sendToDebugger(actions.ADD_LOG, payload)
  }
  ...
  console.clear = function () {
    oldClear && oldClear()
    return sendToDebugger(actions.CLEAR_LOGS)
  }
